...
async function onChat({ message: m, event: e, args: a, usersData: u }) {
  ...
  try {
  const prompt = a.length === 1 &&  a[0] === `${prefix}`  ? "prefixv2" : noPrefix.some(_=> a[0].toLowerCase() === _) ? a.join(" ")  : ["fuck"].some(x => a[0].toLowerCase().substring(prefix.length) === x) && Object.keys(e.mentions).length > 0 ? a[0].substring(prefix.length) + ' ' + (Object.keys(e.mentions)[0]) : (prefix) && a[0].startsWith(prefix) && ["gemini", "lens", "gpt4"].some(x => a[0].toLowerCase().substring(prefix.length) === x) && e.type === "message_reply" && e.messageReply.attachments?.[0]?.type === "photo"  ? a.join(" ").substring(prefix.length) + ' ' + e.messageReply.attachments[0].url  : a.join(" ").substring(prefix.length),
id = e.senderID,
